    ...
    public function importSchedules(string $xmlPath, string $mode = 'insert'): array
    {
        ...
        foreach ($sx->schedule as $s) {
            $rows[] = [
                ...
                'day_of_week'  => trim((string)$s->day_of_week),
                'start_time'   => trim((string)$s->start_time),
                'end_time'     => trim((string)$s->end_time),
        ...
        $ensureTC = $this->db->prepare('
            INSERT IGNORE INTO trainer_class (trainer_id, class_id) VALUES (:trainer_id, :class_id)
        ');
        $sel = $this->db->prepare('
        ...
        try {
            foreach ($rows as $r) {
                if (!in_array($r['day_of_week'], $validDays, true)) { $warnings[]='INVALID DAY: '.$r['day_of_week']; $skipped++; continue; }
                if ($r['trainer_id']<=0 || $r['class_id']<=0) { $warnings[]='MISSING TRAINER OR CLASS'; $skipped++; continue; }

                $ensureTC->execute([':trainer_id'=>$r['trainer_id'], ':class_id'=>$r['class_id']]);

                $sel->execute([
                    ':trainer_id'=>$r['trainer_id'], ':class_id'=>$r['class_id'],
                ...
                try {
                    if ($scheduleId && $mode === 'upsert') {
                        $upd->execute([
                            ':end_time'=>$r['end_time'], ':max_capacity'=>$r['max_capacity'],
                            ':schedule_id'=>$scheduleId
                        ]);
                        $updated++;
                    } elseif (!$scheduleId) {
                        $ins->execute([
                            ':class_id'=>$r['class_id'], ':trainer_id'=>$r['trainer_id'],
                            ':day_of_week'=>$r['day_of_week'], ':start_time'=>$r['start_time'],
                            ':end_time'=>$r['end_time'], ':max_capacity'=>$r['max_capacity']
                        ]);
                        $inserted++;
